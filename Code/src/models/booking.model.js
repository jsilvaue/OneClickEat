...
Booking.create = function (_booking) {
    return new Promise(($resolve, $reject) => {
        dbConn.query("INSERT INTO bookings set ?", _booking, function (err, res) {
            if (err) {
...
Booking.update = function (id, _booking) {
    return new Promise(($resolve, $reject) => {
        dbConn.query("UPDATE bookings SET table_id=?,client_id=?,reservation_date=?,reservation_time=?,status=? WHERE id = ?", [_booking.table_id, _booking.client_id, _booking.reservation_date, _booking.reservation_time, _booking.status, id], function (err, res) {
            if (err) {
